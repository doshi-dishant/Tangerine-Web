	...
	//	dashboardPageCommon.clickOnViewBalance(0);
		...
		if (verifyVisible(dashboardPageCommon.insufficientBalanceText, 2)) {
			...
			if (verifyVisible(dashboardPageCommon.alert, 2)) {
				...
				if (!map.get(MobileNumber).toString().isEmpty())
					verifyVisible(By.xpath(String.format(dashboardPageCommon.verifyMobileNumber, map.get(MobileNumber).toString())),0);
					setLogSteps(this.log,"Verify Mobile Number on Confirmation Screen: " + map.get(MobileNumber).toString());
